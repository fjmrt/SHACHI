...
class G_CNC():
    ...
    def wakeUp(self):
        ...
        self.moveXY('X50','Y21') # offset for Genmitsu 3018PRO-ver V2
        self.sendCommand('G92 X0 Y0 Z0') # set current position 0
    ...
    def moveXY(self,newx,newy,feedspeed = 'default'):
        if feedspeed == 'default':
            feedspeed = self.feedspeed
        if self.zpos != 'Z0':
            self.needleUp()
        if self.zpos == 'Z0':
            command = 'G01 '+newx+' '+newy+' '+'Z0'+' '+feedspeed
